...
1. **Preferred**: Call `plur_admin` MCP tool with `action` = `"plur_inject_hybrid"`, `prompt` = your task description, `scope` = `agent:question-researcher`
2. **Fallback**: If MCP is unavailable, read `.datacore/state/agent-engrams/question-researcher.md` for compiled engrams
